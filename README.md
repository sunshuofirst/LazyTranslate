...
一个功能强大的Chrome浏览器翻译扩展，支持自定义词库和多种翻译API。

## 功能特性

### 🎯 核心功能
- **智能翻译**: 支持整个页面或交互式区域选择翻译
- **自定义词库**: 添加常用词汇的精准翻译
- **多API支持**: 谷歌翻译、微软翻译、百度翻译
- **右键菜单**: 快速访问翻译功能
- **字体联动**: 目标语言与字体智能联动
...
2. **右键菜单功能**
   - LazyTranslate - 翻译此页面：翻译整个网页内容
   - LazyTranslate - 翻译此区域：交互式选择区域进行翻译
...
4. **字体联动功能**
   ...
   - 支持多种语言的专用字体
   - 翻译后自动应用选择的字体
   - **新增**: 使用 CSS class 方式应用字体，便于管理和恢复
...
2. **翻译页面**
   - 方法1：右键点击页面 → "LazyTranslate - 翻译此页面"
   - 方法2：在插件主页面点击"翻译当前页面"

3. **交互式区域选择翻译**
   - 右键点击页面 → "LazyTranslate - 翻译此区域"
   - 移动鼠标，观察不同区域的边框高亮效果
   - 点击想要翻译的区域（如某个段落或div）
   - 观察翻译进度和结果
   - 按ESC键可以退出选择模式
   - **改进**: 现在支持页面滚动，边框指示器始终正确定位
...
2. **字体应用**
   ...
   - 支持页面翻译和区域翻译
   - 字体设置会保存到本地存储
   - **技术实现**: 使用 `lazytranslate-font-applied` CSS class 应用字体
   - **智能恢复**: 显示原文时自动移除字体 class，恢复原始字体
...
│   ├── icon48.png
│   └── icon128.png
├── test_font_linking.html     # 字体联动功能测试页面
└── README.md                  # 说明文档
```
...
### v1.2.0
- **重大改进**: 重新实现元素选择功能，解决页面滚动后定位错误问题
- 新增 `selectElement.js` 文件，使用固定定位和边框指示器
- 改进的区域选择用户体验，支持复杂页面结构
- 优化事件处理，避免内存泄漏
